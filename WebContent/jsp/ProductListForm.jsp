...
function  doEdit(id){
    ...
    document.forms[0].action="<%=request.getContextPath()%>/ProductRetrieve.dog?pId="+id;
    document.forms[0].submit();
}

function doDelete(id){
	if(confirm("คุณต้องการลบรายการ  ID  "+id+" ใช่หรือไม่?")){
			document.forms[0].action="<%=request.getContextPath()%>/ProductDelete.dog?pId="+id;
    		document.forms[0].submit();
	}
}

function onADD(){
	document.forms[0].action="<%=request.getContextPath()%>/ProductAddForm.dog";
    document.forms[0].submit();
}
</script>
</head>
...
<c:url value="/ProductList.dog" var="pagedLink">
	<c:param name="action" value="ProductList.dog"/>
...
<tr><td colspan="5" align="left"><h4>ข้อมูลพื้นฐาน  -> <font color="#f26f21">รายการสินค้า</font></h4></td>
</tr>
<tr bgcolor="#497c09">
	...
	<th style="color:#FFFFFF;font-weight:bold;text-align:center">ประเภทสินค้า</th>
	<th style="color:#FFFFFF;font-weight:bold;text-align:center">ยี่ห้อสินค้า</th>
	<th style="color:#FFFFFF;font-weight:bold;text-align:center">จำนวนสินค้า</th>
	<th style="color:#FFFFFF;font-weight:bold;text-align:center">ราคาต้นทุน</th>
...
<c:forEach items="${pagedListHolder.pageList}" var="item">
    <tr height="20px">
    	...
    	<td>${item.brandProductName}</td>
    	<td>${item.productItem}</td>
    	<td>${item.priceCost}</td>
    	<td>${item.priceSale}</td>
    	<td><a href="javascript:doEdit('${item.productId}')">Edit</a></td>
    	<td><a href="javascript:doDelete('${item.productId}')">Delete</a></td>
    </tr>
</c:forEach>
