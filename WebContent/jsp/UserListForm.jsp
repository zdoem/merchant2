...
function  doEdit(id){
    ...
    document.forms[0].action="<%=request.getContextPath()%>/UsersRetrieve.dog?uId="+id;
    document.forms[0].submit();
}

function doDelete(id){
	if(confirm("คุณต้องการลบรายการ  ID  "+id+" ใช่หรือไม่?")){
			document.forms[0].action="<%=request.getContextPath()%>/UsersDelete.dog?uId="+id;
    		document.forms[0].submit();
	}
}

function onADD(){
	document.forms[0].action="<%=request.getContextPath()%>/UsersAddForm.dog";
    document.forms[0].submit();
}
</script>
</head>
...
<c:url value="/UsersList.dog" var="pagedLink">
	<c:param name="action" value="UsersList.dog"/>
...
<form:form  commandName="UsersForm" method="POST" action="UsersList.dog">
<input type="hidden" name="cmd" value="">
<table width="800px" border="1" bordercolor="#FFFFFF">
<tr><td colspan="5" align="left"><h4>ข้อมูลพื้นฐาน  -> <font color="#f26f21">รายการ Users</font></h4></td>
</tr>
<tr bgcolor="#497c09">
	<th style="color:#FFFFFF;font-weight:bold;text-align:center" width="10%">USER_ID</th>
	<th style="color:#FFFFFF;font-weight:bold;text-align:center">NAME_USER</th>
	...
	<th style="color:#FFFFFF;font-weight:bold;text-align:center">USER_NAME</th>
